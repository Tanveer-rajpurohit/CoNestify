...
export type SortOption = "recently_viewed" | "a_to_z" | "z_to_a" | "newest" | "oldest"
...
const AllFilesList = () => {
  ...
  const [selectedFileTypes, setSelectedFileTypes] = useState<FileType[]>(["canvas", "document", "list"])
  ...
  const filterFiles = () => {
    let filtered = allFiles.filter((file) => file.name.toLowerCase().includes(searchQuery.toLowerCase()))

    // Apply file type filter
    if (selectedFileTypes.length > 0) {
      filtered = filtered.filter((file) => selectedFileTypes.includes(file.type))
    }
    ...
    switch (dateFilter) {
      case "today":
        ...
        break
      case "yesterday":
        filtered = filtered.filter((file) => file.createdDate >= yesterday && file.createdDate < today)
        break
      case "last_7_days":
  ...
  const sortFiles = (files: FileItem[]) => {
    ...
    switch (sortOption) {
      ...
      case "z_to_a":
        ...
        break
      case "newest":
        sortedFiles.sort((a, b) => b.createdDate.getTime() - a.createdDate.getTime())
        break
      case "oldest":
        sortedFiles.sort((a, b) => a.createdDate.getTime() - b.createdDate.getTime())
        break
      case "recently_viewed":
      default: {
        // Sort by viewed date (today, yesterday, older)
        const priority: Record<ViewedDate, number> = { today: 1, yesterday: 2, older: 3 }
        sortedFiles.sort((a, b) => (priority[a.viewedDate] ?? 0) - (priority[b.viewedDate] ?? 0))
        break
      }
    }
  ...
  return (
    <div className="p-6 md:p-8 lg:p-10 relative lg:px-28 h-full">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 p-3 rounded-lg">
        {/* File Type Filter */}
        <FileTypeFilterDropdown selectedFileTypes={selectedFileTypes} toggleFileType={toggleFileTypeSelection} />

        {/* Date Filter */}
        <DateFilterDropdown selectedDateFilter={dateFilter} setDateFilter={setDateFilter} />

       
        <div className="ml-auto flex items-center gap-2">
          {/* Sort Dropdown */}
          <SortDropdown selectedSortOption={sortOption} setSortOption={setSortOption} />
                ...
                <div className="rounded-lg bg-white border border-gray-200 flex flex-col shadow-sm overflow-hidden">
                  {files.map((file) => (
                    ...
                    >
                      ...
                      >
                        <div className="scale-125">{fileTypeData[file.type].icon}</div>
                      </div>
                      <div className="flex flex-col gap-1 flex-grow">
                        <span className="truncate text-base font-medium text-gray-800">{file.name}</span>
                        <span className="truncate text-xs text-gray-500">
                          {file.sharedBy && `Shared by ${file.sharedBy}`} · {file.createdDate.toLocaleDateString()}
                        </span>
                      </div>
                    </div>
                  ))}
